...
st.caption("防守減碼：三大法人賣超、融資維持率惡化、期權籌碼偏空與散戶過度樂觀。加碼：危機解除配合均線支撐。")
...
with tab1:
    ...
    c2.metric("融資維持率", f"{curr_maint:.1f}%", "警戒線: 152%", delta_color="inverse" if curr_maint < 152 else "normal")
    ...
    if is_add_today:
        st.info("當前訊號：籌碼危機解除與均線支撐加碼點！法人與融資籌碼壓力已過，且價格回測至均線支撐帶，建議分批低接。")
    elif trigger_count == 0:
        st.success("當前燈號：綠燈 (籌碼結構安全)。三大法人與期權散戶結構健康，可維持滿倉。")
    elif trigger_count == 1:
        st.warning("當前燈號：黃燈 (籌碼鬆動警戒)。觸發項目：" + "；".join(triggers) + "。建議動作：停止融資開槓桿，提高現金。")
    elif trigger_count == 2:
        st.error("當前燈號：橘燈 (籌碼惡化減碼)。觸發項目：" + "；".join(triggers) + "。建議動作：現貨減碼 50%，清空槓桿。")
    else:
        st.error("當前燈號：紅燈 (流動性與融資斷頭危機/清倉)。觸發項目：" + "；".join(triggers) + "。建議動作：執行無條件清倉避險。")
