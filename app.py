...
try:
    ...
    def load_local_data():
        ...
        raw_df.columns = raw_df.columns.str.strip()  # 清除欄位名稱空白
    ...
    if df.empty:
        st.error("❌ 數據清洗後為空值，請檢查您的 `data.csv` 內容或行數是否足夠計算 MA240。")
        st.stop()

    # ==============================================================================
    # 3. 雙策略動態回測引擎
    # ==============================================================================
    def run_local_backtest(ma_column_name):
        ...
        ma_vals = df[ma_column_name].values
        ...
        for i in range(len(df)):
            ...
            if in_position:
                if c_taiex < c_ma:
                    ...
                    etf_shares = 0.0
                    in_position = False
                    logs.append(f"{c_date.strftime('%Y-%m-%d')} | 🚨 跌破 {ma_column_name}，全數強制清倉避險！")
    ...
    with col_log1:
        ...
        if logs_240:
            for log_str in logs_240[::-1]: st.text(log_str)
        else: st.caption("此配置下未觸發交易。")

    with col_log2:
        st.subheader("📜 120MA 策略交易歷史")
        if logs_120:
            for log_str in logs_120[::-1]: st.text(log_str)
        else: st.caption("此配置下未觸發交易。")
